...
class ProjectProject(models.Model):
    ...
    def _compute_allowed_partner(self):
        for rec in self:
            rec.allowed_partner_ids = rec.collaborator_ids.mapped('partner_id').ids

    allowed_partner_ids = fields.Many2many(
        'res.partner', 'project_allowed_partner_rel', 'collaborator_id', 'partner_id',
        compute="_compute_allowed_partner", store=True
    ) # compute='_compute_allowed_users', inverse='_inverse_allowed_user'
