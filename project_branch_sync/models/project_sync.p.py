...
class MailChannel(models.Model):
    ...
    def check_git_login(self):
        ...
        if "vertelbot" not in self.run(["git", "config", "--global", "user.name"], capture_output=True, text=True).stdout:
            self.run(["git", "config", "--global", "user.name", "vertelbot"], capture_output=True, text=True)    
    
    def send_email(self, result):
        test = self.committer_email if self.committer_email else 'vertelbot@vertel.se'
