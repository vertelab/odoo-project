...
class GitHubWebHooks(http.Controller):
    ...
    def task_push(self, **payload):
        ...
        _logger.warning(f"Task number: {task_number=} ")
  ...
  # ~ }
        ...
        user = request.env['res.users'].sudo().search([
                    '|', '|', '|',
                    ('email', '=', committer_email),  # Exact match
                    ('email', 'ilike', committer_email),  # Case-insensitive match
                    ('email', 'ilike', committer_email.strip()),  # Case-insensitive with stripped input
                    ('email_normalized', '=', committer_email)  # Match normalized email (if applicable)
                ], limit=1)
        task = request.env['project.task'].sudo().search([('number','=',task_number)],limit=1)
        if not task:
            project = request.env['project.project'].sudo().search([('name','=',repo)],limit=1)
            if not project:
                project = request.env['project.project'].sudo().create({'name': repo})
            user_ids = [(6,0,[user.id])] if user else None
            task = request.env['project.task'].sudo().create({'project_id': project.id, 'name': message,'number': task_number,'user_ids': user_ids })
        if not project:
        ...
        _logger.warning(f"Success: {user=} {task=}  {project=} {message=} {task_number=} {foo.body=} {foo.author_id.name=}")
