...
class ProjectProject(models.Model):
        ...
        if self.is_equity_portfolio:
            ...
            self.type_ids = [
                (
                    ...
                    0,
                    [
                        self.env.ref("project_equity_portfolio.task_type_active"),
                        self.env.ref("project_equity_portfolio.task_type_inactive"),
                    ],
                )
            ]
        else:
    ...
    def _compute_value(self):
        for portfolio in self:
            latest_valuation = self.env["project.equity_valuation"].search(
                [("project_id", "=", portfolio.id)], order="date desc", limit=1
            )
    ...
    def _compute_account(self):
        for portfolio in self:
            portfolio.equity_account = sum(
                portfolio.transaction_ids.mapped("equity_value")
            )
    ...
    @api.depends(
        "transaction_ids", "transaction_ids.date", "valuation_ids", "valuation_ids.date"
    )
    ...
    valuation_count = fields.Integer(
        string="Valuation Count", compute="_compute_valuation_count"
    )
    ...
    def equity_buy(self, equity_symbol, number, value, transaction_fee):
        for portfolio in self:
            equities = portfolio.task_ids.filtered(
                lambda e: e.equity_symbol == equity_symbol
            )
            if not equities:
                equity = self.env["project.task"].create(
                    {
                        "name": equity_symbol,
                        "equity_symbol": equity_symbol,
                        "project_id": portfolio.id,
                        "equity_number": number,
                    }
                )
                price_transaction = self.env["project.task.equity_price"].create(
                    {"task_id": equity.id, "price": value / number}
                )
            else:
                equity = equities[0]
            equity.equity_number += number
            if equity.stage_id != self.env.ref(
                "project_equity_portfolio.task_type_active"
            ):
                equity.stage_id = self.env.ref(
                    "project_equity_portfolio.task_type_active"
                )
    ...
    def equity_sell(self, equity_symbol, number, value, transaction_fee):
        for portfolio in self:
            ...
            if equity.equity_number <= 0.0:
                equity.stage_id = self.env.ref(
                    "project_equity_portfolio.task_type_inactive"
                )
            transaction = self.env["project.equity_transaction"].create(
                {
    ...
    equity_research_rss = fields.Char(
        string="Rss-feed", default="https://www.placera.se/artiklar/rss.xml"
    )
            ...
            if portfolio.is_equity_portfolio:
                ...
                if not feed.bozo:
                    ...
                    for entry in feed.entries:
                        ...
                        if (
                            self.env["project.equity_research"].search_count(
                                [
                                    ("project_id", "=", portfolio.id),
                                    ("link", "=", entry.link),
                                ]
                    ...
                    _logger.error(
                        f"Could not read the feed {portfolio.equity_research_rss}"
                    )
...
class ProjectProjectValuationRecord(models.Model):
    ...
    date = fields.Date(
        string="Valuation Date", default=fields.Date.context_today, required=True
    )
    ...
    def _compute_valuation(self):
        for portfolio in self.env["project.project"].search(
            [("is_equity_portfolio", "=", True)]
        ):
            self.env["project.equity_valuation"].create(
                {
                    "project_id": portfolio.id,
                    "value": sum(
                        [task.price * task.number for task in portfolio.task_ids]
                    ),
...
class ProjectEquityResearch(models.Model):
    ...
    project_id = fields.Many2one(
        "project.project", string="Portfolio", required=True, ondelete="cascade"
    )
    desc = fields.Text(
    ...
    topic_id = fields.Many2one(
        comodel_name="project.equity_research.topic", string="Topic", help=""
    )  # domain|context|ondelete="'set null', 'restrict', 'cascade'"|auto_join|delegatefields.Char(string='Media', required=True)
    date = fields.Date(
        string="Research Date",
    ...
    def get_latest_news(self):
        ...
        for topic in self:
            googlenews = GoogleNews(
                lang=topic.project_id.equity_research_lang,
