...
class ModuleMonitor(models.TransientModel):
    ...
    git_repo_ids = fields.Many2many('git.repos', string="Git Repos", domain="[('author', '=', module_author)]")
    ...
    def load_modules(self):
        ...
        for git_repo in self.git_repo_ids:
            ...
            try:
                ...
                repo = g.get_repo(f"{self.module_author}/{git_repo.name}")
            except Exception as e:
                raise UserError(f"Could not read {self.module_author}/{git_repo.name} {e}")

            branches = [b.name for b in repo.get_branches() if re.match("^\d*[.]0$", b.name)]
            for branch in branches:
                stage = self.env['project.task.type'].search([('project_ids','in',self.project_id.id),('name','=',branch)])
                if not stage:
            ...
            for content_file in contents:
                ...
                if content_file.type == "dir":
                    ...
                    for b in branches:
                        self.message_box = f"{git_repo.name} {content_file.name} {b}"
                        _logger.info(self.message_box)
