...
class WcagRule(models.Model):
    ...
    def create(self, vals_list):
        ...
        res = super(WcagRule, self).create(vals_list)
        return res
...
class WcagProjectRule(models.Model):
    ...
    def create(self, vals_list):
        ...
        res = super(WcagProjectRule, self).create(vals_list)
...
class ProjectTaskWcag(models.Model):
    ...
    wcag_url = fields.Char(related='wcag_id.wcag_url', store=True, aggregator="max")
    ...
    wcag_state = fields.Selection(
            ...
            default=False,
            aggregator="max")
    notes = fields.Text(String = "Notes", aggregator="max")
    extra_notes = fields.Text(String = "Extra Notes", aggregator="max")
    display_wcag_name = fields.Char()
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if vals.get('wcag_project_id'):
                 ...
                 vals['display_wcag_name'] = self.env['wcag.project.rule'].browse(
                     vals['wcag_project_id']
                 ).display_wcag_name
            else:
                vals['wcag_id'] = False
        res = super(ProjectTaskWcag, self).create(vals_list)
        return res
